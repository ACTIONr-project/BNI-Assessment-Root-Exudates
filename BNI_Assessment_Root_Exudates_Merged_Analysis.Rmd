...
# Let's see which dataframes are inside each list
rownames(as.matrix(summary(inhibition_barplots)))
```
...
Step 2: We will investigate the distribution of the AOI values in order to decide whether to proceed with parametric or non - parametric tests. 
  
```{r echo=TRUE, message=FALSE, warning=FALSE}
# A Shapiro - Wilk test 
shapiro.test(data_statistics$AOI)

# We can create a histogram
AOI_histogram <- hist(data_statistics$AOI, breaks = 9, main = "Histogram of Ammonia Oxidation Inhibition % values", xlab = "Ammonia Oxidation Inhibition %", col = "skyblue") 

# We can also utilise a Q-Q plot
qqnorm(data_statistics$AOI, main = "Q-Q plot of Ammonia Oxidation Inhibition % values", col = "skyblue4")
qqline(data_statistics$AOI)
